...
public final class DepthDataProcessor {
    ...
    public struct DepthMapOutput {
        ...
        public let rawData: Data // Float32 array of millimeters (or meters)
        public let minDepthMm: Float
        public let maxDepthMm: Float
        public let validPointCount: Int
    }
    
    /// Converts AVDepthData to calibrated Float32 millimeter buffer
    public static func processDepthData(_ depthData: AVDepthData) -> DepthMapOutput? {
            ...
            for y in 0..<height {
                ...
                for x in 0..<width {
                    ...
                    if rawVal.isFinite && rawVal > 0.15 && rawVal < 1.2 { // Valid face range: 15cm - 120cm
                        let depthMm = rawVal * 1000.0 // convert meters to millimeters
                        outFloats[idx] = depthMm
                        if depthMm < minD { minD = depthMm }
                        if depthMm > maxD { maxD = depthMm }
                        validPoints += 1
                    } else {
        ...
        return DepthMapOutput(
            ...
            height: height,
            rawData: outputData,
            minDepthMm: minD == Float.greatestFiniteMagnitude ? 0 : minD,
            maxDepthMm: maxD,
