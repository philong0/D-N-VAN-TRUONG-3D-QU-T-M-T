...
# Direction resolved by DIRECT EMPIRICAL TEST (sweep_directions.py rendered
# all 6 cardinal directions with a bright background at cheap res and
# compared pixel std-dev/silhouette size) -- the mesh's own "nose" vertex
# normals pointed INWARD (opposite of the true outward-facing direction,
# most likely a winding-order artifact from the custom trimesh GLB export
# in export_glb.py, not trustworthy), which is why the earlier basis-from-
# normal attempt kept producing an edge-on sliver. +Y empirically produced
# the correct, clearly recognizable frontal face silhouette; -Y (the
# nose-normal-implied direction) did not. World Z is confirmed up (glTF
# Y-up -> Blender Z-up conversion, already verified via the mesh's own
# per-axis bounding box: Z has the largest span = height).
...
bg.inputs[1].default_value = p["ambient"]
