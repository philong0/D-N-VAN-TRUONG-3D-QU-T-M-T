...
export default async function PatientDossierPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const labels = getSlotLabels(patient.services, patient.scanPhotoMapping);
  ...
  return <div className="mx-auto w-full max-w-5xl px-4 py-5 sm:px-6 sm:py-8 lg:px-10 lg:py-10">
    ...
    {patient.optionalClinicalPhotos?.legacy_angle4 && <section className="mt-5 rounded-2xl border border-border p-4">
      <p className="mb-2 text-sm text-muted">Ảnh góc 4 trước đây / ảnh lâm sàng bổ sung — không thuộc reconstruction scan.</p>
      <img className="max-h-64 rounded-xl" alt="Ảnh lâm sàng lưu riêng" src={`/api/files/${patient.id}/photos/${patient.optionalClinicalPhotos.legacy_angle4.fileName}`} />
    </section>}
